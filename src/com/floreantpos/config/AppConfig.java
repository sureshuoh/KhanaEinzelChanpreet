...
public class AppConfig {
	...
	static {
		try {
			...
			File configFile = new File("khana.config.properties"); //$NON-NLS-1$
			...
			File directory = new File("C:/Backup/Config");
	    	if (!directory.exists()) {
				directory.mkdirs();
			}
	    	
	    	Files.deleteIfExists(new File("unins000.exe").toPath());
	    	Files.deleteIfExists(new File("unins000.dat").toPath());
	    	
			File bckpconfigFile = new File(directory.toString()+"/khana.config_bckp.properties"); //$NON-NLS-1$
			try {
			if(config.getBoolean("SUPER_ADMIN")||!config.getBoolean("SUPER_ADMIN"))
				Files.copy(configFile.toPath(), bckpconfigFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
			}catch(Exception ex) {			
				if((bckpconfigFile.exists())) {
					config = new PropertiesConfiguration(bckpconfigFile);			
					config.setAutoSave(true);
					Files.copy(bckpconfigFile.toPath(), configFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
				}			
