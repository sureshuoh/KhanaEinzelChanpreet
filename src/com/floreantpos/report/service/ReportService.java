...
public class ReportService {
	...
	private static SimpleDateFormat shortDateFormat = new SimpleDateFormat("hh:mm dd MM");
	...
	public MenuUsageReport getMenuUsageReport(Date fromDate, Date toDate, int type, boolean flag) {
		if(type == 1)
			return getCategoryReport(fromDate,toDate,flag);
		else if(type == 2)
			return getGroupReport(fromDate,toDate,flag);
		else
			return getItemReport(fromDate,toDate,flag);
	}
	
	
	public MenuUsageReport getMenuUsageReportTicketIts(Date fromDate, Date toDate, int type, boolean flag, List<TicketItem> items) {
		if(type == 1)
			return getCategoryReportByItem(fromDate,toDate,flag, items);
		else if(type == 2)
			return getGroupReport(fromDate,toDate,flag);
		else
			return getItemReport(fromDate,toDate,flag);
	...
	public MenuUsageReport getItemReport(Date fromDate, Date toDate,boolean flag)
	{
		...
		try {
			...
			List<MenuItem> items = criteria.list();
			MenuItem miscCategory = new MenuItem();
			for (MenuItem item : items) {
				...
				criteria.add(Restrictions.eq("item." + TicketItem.PROP_NAME, item.getName()));
				...
				if (datas.size() > 0) {
					...
					data.setCategoryName(item.getName());
	...
	public MenuUsageReport getCategoryReport(Date fromDate, Date toDate,boolean flag)
	{

		int nineTeen =TaxDAO.getInstance().findByName(POSConstants.DINE_IN).getRate().intValue();
		int seven = TaxDAO.getInstance().findByName(POSConstants.HOME_DELIVERY).getRate().intValue();		
		GenericDAO dao = new GenericDAO();
		MenuUsageReport report = new MenuUsageReport();
		...
		try {
			...
			MenuCategory misc = new MenuCategory();
			misc.setName(com.floreantpos.POSConstants.MISC);
			misc.setType(POSConstants.DINE_IN);
			misc.setBeverage(true);
			misc.setVisible(true);
			categories.add(misc);  

			if (TerminalConfig.isSupermarket()) {
				MenuCategory misc7 = new MenuCategory();
				misc7.setName("SONSTIGES_");
				misc7.setType(POSConstants.HOME_DELIVERY);
				misc7.setBeverage(true);
				misc7.setVisible(true);
				categories.add(misc7);
			}
			...
			for (MenuCategory category : categories) {
				...
				criteria.add(Restrictions.le("t." + Ticket.PROP_CREATE_DATE, toDate));
				criteria.add(Restrictions.eq("t." + Ticket.PROP_PAID, Boolean.TRUE));
				//				criteria.add(Restrictions.eq("t." + Ticket.PROP_DRAWER_RESETTED, flag));
				...
				if (datas.size() > 0) {
					...
					if(category.getType().equals(POSConstants.DINE_IN))
						data.setTax(nineTeen);
					else if(category.getType().equals(POSConstants.HOME_DELIVERY))
						data.setTax(seven);
					else
						data.setTax(0);
	...
	public MenuUsageReport getCategoryReportByItem(Date fromDate, Date toDate,boolean flag, List<TicketItem> itemList)
	{
...
//			List<TicketItem> itemList =TicketItemDAO.getInstance().findByDate(fromDate, toDate);


			for (MenuCategory category : categories) {
				MenuUsageReportData data = new MenuUsageReportData();				
				int count = 0;
				double grosSales = 0.0;
				double discount = 0.0;
				if(category==null||category.getName()==null)
					continue;
				if(category.getType().equals(POSConstants.DINE_IN)) {
					List<TicketItem> temList = itemList.stream().filter(TicketItem->TicketItem.getCategoryName()!=null&&TicketItem.getCategoryName().equals(category.getName())&&TicketItem.getTaxRate()==nineTeen).collect(Collectors.toList());
					for(TicketItem Item:temList) {
							count += Item.getItemCount();
							grosSales += Item.getTotalAmount();
							discount += Item.getDiscountAmount();
					}
					data.setGrossSales(grosSales);
					...
					if(grosSales>0)
						report.addReportData(data);
					data = new MenuUsageReportData();	
					temList = itemList.stream().filter(TicketItem->TicketItem.getCategoryName()!=null&&TicketItem.getCategoryName().equals(category.getName())&&TicketItem.getTaxRate()==seven).collect(Collectors.toList());
					count = 0;
					grosSales = 0.0;
					discount = 0.0;
					for(TicketItem Item:temList) {
					...
					if(grosSales>0)
						report.addReportData(data);
				}else if(category.getType().equals("ZERO")) {
					List<TicketItem> temList = itemList.stream().filter(TicketItem->TicketItem.getCategoryName()!=null&&TicketItem.getCategoryName().equals(category.getName())&&TicketItem.getTaxRate()==0).collect(Collectors.toList());
					for(TicketItem Item:temList) {
							count += Item.getItemCount();
							grosSales += Item.getTotalAmount();
							discount += Item.getDiscountAmount();
	...
	public MenuUsageReport getGroupReport(Date fromDate, Date toDate,boolean flag)
	{
		GenericDAO dao = new GenericDAO();
		MenuUsageReport report = new MenuUsageReport();
		Session session = null;
		...
		try {
			...
			MenuGroup miscCategory = new MenuGroup();
			for (MenuGroup group : groups) {
				...
				if (datas.size() > 0) {
					...
					MenuUsageReportData data = new MenuUsageReportData();
					data.setCategoryName(group.getName());
