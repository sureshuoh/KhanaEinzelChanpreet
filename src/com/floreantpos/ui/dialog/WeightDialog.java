...
public class WeightDialog extends POSDialog implements ActionListener{

	static Double weight;
	static Double price;
	String name;
	String id;
	private static FixedLengthTextField tfWeight;
	PosButton okButton;
	PosButton cancelButton;
	Double netPrice = 0.00;
	Double weight_given = 0.00;
	PosButton btnClear;
	static JLabel lblDisplay;
	public static ScaleWeight serial;
	static JLabel lblKg;
	static PosButton btnWeight;

	
	public WeightDialog(Double weight, Double price,String name,String id)
	{
		this.weight = weight;
		this.price = price;
		this.name = name;		
	...
	public void init()
	{
		        ...
		        if (e.getID() == KeyEvent.KEY_PRESSED) {
		           ...
		           if (e.getKeyCode() == KeyEvent.VK_ENTER) {
		        	   		...
		        	   		if(tfWeight.getText() == null || tfWeight.getText().length() == 0)
							{
		        	   		...
		        	   		weight_given = Double.parseDouble(tfWeight.getText().replace(",", "."))*1000;
		...
		tfWeight = new com.floreantpos.swing.FixedLengthTextField(10);
		//tfWeight.setDocument(new FixedLengthDocument(50));
		tfWeight.setBackground(new Color(128,207,226));
		tfWeight.setBorder(null);
		//tfWeight.setFocusable(true);
...
//					System.out.println(tfWeight.getText().length());					
					weight_given = Double.parseDouble(tfWeight.getText().replace(",", "."))*1000;
					netPrice = (weight_given/weight) * price;
					netPrice = Math.floor(netPrice * 100) / 100;
					netPrice = (netPrice*100)/100;
		...
		btnWeight = new PosButton();
		btnWeight.setIcon(IconFactory.getIcon("waage.png"));
		...
		btnWeight.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				/*if(TerminalConfig.isWeightClient()) {
					try {
						setWeight(GetWeightClient.getWeight("http://"+TerminalConfig.getWeightServerIp()+":8000/getWeight"));
					} catch (Exception e1) {
				...
				if(!TerminalConfig.isWaageDisable()) {
					...
					btnWeight.setBackground(new Color(232, 49, 32));
		...
		lbl2.setText(NumberUtil.formatNumber(price) + "€ / " + weight  + " gram");
		...
		lbl2.addMouseListener(new MouseListener() {
			...
			public void mousePressed(MouseEvent me) {
				if (me.getClickCount() == 2) {
					ErrorMessageDialog dialog = new ErrorMessageDialog("Wollen SiePreis Aendern??", true, true);
					dialog.pack();
					dialog.open();
					if(dialog.isCancelled())return;
					price = NumberSelectionDialog2.takeDoubleInput("Geben Sie die Preis/kg: "+ NumberUtil.formatNumber(price) + " €","Aendern Preis");					
					lbl2.setText(NumberUtil.formatNumber(price) + "€ / " + weight  + " gram");
					updatePriceView();
				}
			}
		...
		textPanel.add(lbl1, "cell 0 0 4 0");
		...
		textPanel.add(lbl2, "cell 0 0 5 0, wrap");
