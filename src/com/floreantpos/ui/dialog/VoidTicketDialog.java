...
public class VoidTicketDialog extends POSDialog {
	...
	private void initComponents() {
		...
		titlePanel1.setBackground(new Color(209,222,235));
		...
		ticketDetailView = new com.floreantpos.ui.views.TicketDetailView(true);
		transparentPanel2 = new com.floreantpos.swing.TransparentPanel();
		transparentPanel2.setBackground(new Color(209,222,235));
		cbVoidReasons = new javax.swing.JComboBox();
		cbVoidReasons.setBackground(Color.WHITE);
		btnNewVoidReason = new com.floreantpos.swing.PosButton();
		
		btnVoidTicketSearch = new com.floreantpos.swing.PosButton();

		btnVoidTicketSearch.setText(POSConstants.SEARCH);
		btnVoidTicketSearch.addActionListener(new java.awt.event.ActionListener() {
			@Override
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				int ticketId = NumberSelectionDialog2.takeIntInput("Bitte Rechnug Nummer eingeben!!!");
				if (ticketId!=0) {
					Ticket ticket1 = TicketDAO.getInstance().findTicketByTicketId(ticketId);
					if(ticket1!=null)
						setTicket(ticket1);
				}
			}
		...
		chkItemsWasted.setBackground(new Color(209,222,235));
		jLabel1 = new javax.swing.JLabel();
		transparentPanel3 = new com.floreantpos.swing.TransparentPanel();
		transparentPanel3.setBackground(new Color(209,222,235));
		transparentPanel4 = new com.floreantpos.swing.TransparentPanel();
		transparentPanel4.setBackground(new Color(209,222,235));
		...
		btnVoid.setBackground(new Color(102,255,102));
	...
	private void btnNewVoidReasonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNewVoidReasonActionPerformed
		try {
			...
			if (!dialog.isCanceled()) {
				...
				VoidReasonDAO dao = new VoidReasonDAO();
				String reason = voidReason.toString();
				if (reason.compareTo(terminalId) != 0)
				{
					dao.save(voidReason);
				}
	...
	private void btnVoidActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnVoidActionPerformed
		try {
			...
			if(voidR != null)
			{
				if(voidR.toString().compareTo(terminalNumber.trim()) == 0)
				{
					if(ticket.getCashPayment() != null && !ticket.getCashPayment()) {
						POSMessageDialog.showError("Dieses Ticket wurde mit der Karte bezahlt");
						dispose();
						return;
					}
					...
					ticket.setTicketItems(Arrays.asList(TicketItem.newZeroTicketItem()));
					ticket.calculatePrice();
					ticket.setDrawerResetted(true);
					TicketDAO.getInstance().saveOrUpdate(ticket);

					ticketItems.stream().forEach(
							ticketItem -> TicketItemDAO.getInstance().delete(
									ticketItem.getId()));
					
					POSMessageDialog.showMessage("Erfolg!!");
					dispose();
					return;
				}
			}
			...
			JReportPrintService.printDeletedTicket(ticket, voidReason.getReasonText());

			try {
				String title = "- VOID RECEIPT -";
				String data = "Ticket # " + (ticket.getTicketid()!=null?ticket.getTicketid():ticket.getId()) + " was voided.";
	...
	private com.floreantpos.swing.PosButton btnVoidTicketSearch;
